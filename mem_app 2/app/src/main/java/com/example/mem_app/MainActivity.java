...
//        login_page = new Login();
    ...
    private void loginHttpSend(){
        LoginHttp login_http = new LoginHttp(this.text_username.getText().toString(), this.text_password.getText().toString());
        login_http.send();
    }

    private Boolean checkUserExistence() throws Exception{
        if (this.text_username.getText().equals(""))
            throw new EmptyUsernameException();
        if (this.text_password.getText().equals(""))
